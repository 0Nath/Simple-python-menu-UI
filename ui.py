...
class Ui:
    def __init__(self, dim:tuple, fps:int, menu: "Case",color1:tuple=(48,48,48),color2:tuple=(35,35,35),back_color:tuple = (0,0,0),font_color:tuple=(255,255,255),font:str="consolas",font_size:int=25,fullscreen:bool = False):
        self.dim = dim
        self.fps = fps
        self.font_str = font
    ...
    def render(self, menu):
        if menu.title.startswith("<DOn>"):
            ...
            for j,i in enumerate(menu.submenu):
                if i.title[:5]=="<DOn>" or i.title[:5]=="<DOa>":
                    title = i.title[5:]
                else:
                    title = i.title
...
def benchmark(screen,back_clolr,font_str,dimention):
    ...
    font = pygame.font.SysFont(font_str,15)
    frames = 0
    last_time = time.time()
    text_fps = "-- FPS"
    while not end:
